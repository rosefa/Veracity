...
'''train = embeddings_train(data_train.article_content)
embeddings_train=np.array([np.reshape(embed, (len(embed), 1)) for embed in train])'''
model = model.fit(embeddings_train,data_train['labels'].values,epochs=50,validation_data=(embeddings_test,data_test['labels'].values),shuffle=True,batch_size=40)   
