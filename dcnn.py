...
def builModel ():
    ...
    model.add(layers.MaxPooling1D())
    #model.add(BatchNormalization())
    model.add(layers.Bidirectional(LSTM(64, recurrente_dropout=0.2)))
    #model.add(Dropout(0.2))
...
embeddings_train=np.array([np.reshape(embed, (len(embed), 1)) for embed in train])'''
model = builModel()
model.fit(embeddings_train,data_train.labels,epochs=100,validation_data=(embeddings_test,data_test.labels),batch_size=40)   
