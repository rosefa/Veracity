...
def build_bilstm(word_index, embeddings_dict, MAX_SEQUENCE_LENGTH=300, EMBEDDING_DIM=100):
    ...
    model7 = layers.add([model1,model2])
    model7 = layers.add([model7,model3])
    model7 = layers.add([model7,model4])
    model7 = layers.add([model7,model5])
...
history = model.fit(myData_train_Glove, y_train,validation_data=(myData_test_Glove, y_test), epochs=50, batch_size=100, verbose=2)
